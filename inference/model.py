...
class MLA(nn.Module):
    ...
    def forward(
        ...
        print_tensor_stats("Before rotary embedding q_pe: ", q_pe)
        q_pe = apply_rotary_emb(q_pe, freqs_cis)
        print_tensor_stats("After rotary embedding q_pe: ", q_pe)
        kv = self.wkv_a(x)
        kv, k_pe = torch.split(kv, [self.kv_lora_rank, self.qk_rope_head_dim], dim=-1)
        print_tensor_stats("Before rotary embedding k_pe: ", k_pe)
        k_pe = apply_rotary_emb(k_pe.unsqueeze(2), freqs_cis)
        print_tensor_stats("After rotary embedding k_pe: ", k_pe)
        if attn_impl == "naive":
        ...
        print_tensor_stats("After attention wo: ", x)
        return x
